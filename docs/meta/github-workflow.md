...
**모든 작업은 이슈에서 시작하고, 브랜치·커밋·PR을 그 이슈에 연결한다.** 이슈 없는 작업 금지(사소하면 `type:chore` 이슈). 커밋 footer에 `Refs: #N`(관련) 또는 `Closes #N`(완료)를 반드시 넣는다.

## 이슈 → 보드 → 브랜치 → PR → 머지 (흐름)

1. **이슈 생성**: 백로그 항목을 이슈로. 템플릿(feature/spec/design/bug) 사용, 라벨·Milestone·Project 지정.
2. **보드 반영**: Project 보드에서 Todo → In Progress로 이동(착수 시).
3. **브랜치**: 이슈 번호 + 스코프 — `feat/12-import-parser`, `docs/8-requirements-baseline`, `fix/...`.
4. **커밋**: [`commit-conventions`](../../.claude/skills/commit-conventions/SKILL.md) 준수. footer에 이슈 링크 필수 — `Refs: #12` 또는 `Closes #12`.
